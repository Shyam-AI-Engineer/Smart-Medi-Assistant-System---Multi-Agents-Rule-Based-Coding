...
@router.get(
    "/{patient_id}/export",
    status_code=status.HTTP_200_OK,
    summary="Export vitals as PDF",
    description="Download a PDF vitals history report for the patient",
)
def export_vitals_pdf(
    ...
    """
    Generate and download a PDF vitals report.

    - Patients may only export their own data.
    - Doctors and admins may export any patient.
    ...
    # Access control: patient can only export their own records
    ...
    # Fetch all vitals (newest first → reverse for report)
    vitals_qs = (
        db.query(Vitals)
